...
#
# Nomes das séries por NCM:
# O nome técnico de cada série NCM é curto por design:
# "{nome_curto}_{exp|imp}_{fob|kg}" (sem o código NCM e sem
# o prefixo "comex_"), porque a etapa seguinte do pipeline já
# prefixa cada variável com o nome da fonte (ex.: "comexstat_").
# O código NCM continua disponível em dicionario_produtos_ncm
# e na coluna codigo_ncm do dicionário de variáveis — não se
# perde rastreabilidade, só sai do nome da coluna.
#
# Saídas:
...
# 3. Defina "ativo": True.
# 4. O nome_curto precisa ser único entre as NCMs ativas —
#    ele agora é a única parte do nome da série que identifica
#    o produto (o código NCM não entra mais no nome da coluna).
#
...
def obter_codigos_ncm_ativos(
    ...
    return codigos
def validar_nomes_curtos_unicos(
    produtos_ncm: dict,
) -> None:
    """
    Garante que não há nome_curto repetido entre as NCMs
    ativas.

    Como o código NCM não faz mais parte do nome da série
    (ver obter_nome_serie_ncm), um nome_curto duplicado
    geraria duas séries diferentes com o mesmo nome de
    coluna — uma sobrescreveria a outra silenciosamente no
    pivot_table de montar_base_ncm_larga.
    """
    nomes = [
        metadados["nome_curto"]
        for metadados in produtos_ncm.values()
        if metadados.get("ativo", False)
    ]
    duplicados = sorted({
        nome
    ...
    if duplicados:
        raise ValueError(
            "nome_curto duplicado entre NCMs ativas: "
            f"{duplicados}. Cada produto ativo precisa de "
            "um nome_curto único."
        )
...
def obter_nome_serie_ncm(
    ...
    O código NCM não entra no nome (fica só no dicionário de
    produtos, via a coluna codigo_ncm/produto do dicionário de
    variáveis); o nome fica curto porque a etapa seguinte do
    pipeline já prefixa cada variável com o nome da fonte
    (ex.: "comexstat_"). Formato final:
    "{nome_curto}_{exp|imp}_{fob|kg}".
    """
    if fluxo == "exportacao":
        fluxo_abrev = "exp"
    ...
    return f"{nome_curto}_{fluxo_abrev}_{metrica_abrev}"
...
def validar_base(
    ...
    periodo_final: str,
    config: dict,
    colunas_ncm: list[str],
) -> list[str]:
    """
    Valida a base completa.
    Retorna avisos não impeditivos.

    colunas_ncm: nomes das séries NCM (vindos de
    metadados_ncm.keys()), usados para checar valores
    negativos. Como o nome da série não carrega mais um
    prefixo fixo do tipo "comex_ncm_", as colunas NCM
    precisam ser informadas explicitamente em vez de
    detectadas por padrão de texto.
    """
    avisos = []
    if base.empty:
        raise ValueError(
    ...
    colunas_ncm_presentes = [
        coluna
        for coluna in colunas_ncm
        if coluna in base.columns
    ]
    if colunas_ncm_presentes:
        negativos_ncm = (
            base[colunas_ncm_presentes] < 0
        ).any().any()
        if negativos_ncm:
            raise ValueError(
...
def atualizar_base_comexstat(
    ...
    try:
        ...
        avisos = validar_base(
            ...
            periodo_final=periodo_final,
            config=config_execucao,
            colunas_ncm=list(metadados_ncm.keys()),
        )
        # ----------------------------------------------------
        # Exportações
        # ----------------------------------------------------
